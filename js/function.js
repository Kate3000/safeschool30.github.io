;(function() {
	...
	class Controller {
		...
		init() {
			...
			if (this.player === human) {
				compShot = false;
				this.text = 'Ви стріляєте першим';
			} else {
				compShot = true;
				this.text = 'Першим стріляє комп\'ютер';
		...
		makeShot(e) {
			...
			switch(v) {
				...
				case 3:
				case 4:
					Controller.showServiceText('По цим координатам ви вже стріляли!');
					break;
			}
		}
		...
		miss(x, y) {
			...
			if (this.player === human) {
				text = 'Ви промахнулись. Стріляє вірус.';
				this.player = computer;
				this.opponent = human;
				compShot = true;
				setTimeout(() => this.makeShot(), 2000);
			} else {
				text = 'Вірус промахнувся. Ваш постріл.';
		...
		hit(x, y) {
			...
			text = (this.player === human) ? 'Вітаємо! Ви попали. Ваш постріл.' : 'Вірус в вас попав. Постріл віруса';
			setTimeout(() => Controller.showServiceText(text), 400);
	...
	buttonPlay.addEventListener('click', function(e) {
		...
		toptext.innerHTML = 'Вірус та профілактика';
	...
	buttonNewGame.addEventListener('click', function(e) {
		...
		human.cleanField();
		toptext.innerHTML = 'Розстановка кораблів';
